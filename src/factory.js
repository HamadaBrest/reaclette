...
const EMPTY = Object.freeze({ __proto__: null });
const {
  create,
  defineProperty,
  keys,
...
module.exports = ({ Component }) => {
  function withStore({ computed, effects, initialState }, fn) {
    const computedKeys = computed === undefined ? [] : keys(computed);
    const effectsKeys = effects === undefined ? [] : keys(effects);
    ...
    return class Decorated extends React.PureComponent {
      constructor(props) {
        ...
        let state = (this._state =
          initialState === undefined ? EMPTY : initialState(props));
